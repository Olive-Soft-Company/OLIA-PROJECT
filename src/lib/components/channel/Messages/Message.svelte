			...
			<div class="relative text-xs mb-1">
				...
				>
					{#if message?.reply_to_message?.meta?.model_id}
						<img
							src={`${OLIA_API_BASE_URL}/models/model/profile/image?id=${message.reply_to_message.meta.model_id}`}
					...
					{:else}
						<img
							src={message.reply_to_message.user?.role === 'webhook'
								? `${OLIA_API_BASE_URL}/channels/webhooks/${message.reply_to_message.user?.id}/profile/image`
								: `${OLIA_API_BASE_URL}/users/${message.reply_to_message.user?.id}/profile/image`}
			...
			<div class={`shrink-0 mr-1 w-9`}>
				{#if showUserProfile}
					{#if message?.meta?.model_id}
						<img
							src={`${OLIA_API_BASE_URL}/models/model/profile/image?id=${message.meta.model_id}`}
						...
						<ProfilePreview user={message.user}>
							<ProfileImage
								src={message.user?.role === 'webhook'
									? `${OLIA_API_BASE_URL}/channels/webhooks/${message.user?.id}/profile/image`
									: `${OLIA_API_BASE_URL}/users/${message.user?.id}/profile/image`}
						...
						{#each message?.data?.files as file}
							{@const fileUrl =
								file.url.startsWith('data') || file.url.startsWith('http')
									? file.url
									: `${OLIA_API_BASE_URL}/files/${file.url}${file?.content_type ? '/content' : ''}`}
							<div>
