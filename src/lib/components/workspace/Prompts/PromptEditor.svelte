<script lang="ts">
	...
	// i18n is a store in context in your project (since you use $i18n)
	...
	// IMPORTANT: keep TS assertions OUT of the template; do them here instead
	...
	const submitHandler = async () => {
		...
		try {
			if (validateCommandString(command)) {
				...
				if (history.length > 0) selectedHistoryEntry = history[0];
			} else {
				toast.error(
					$i18n.t('Only alphanumeric characters and hyphens are allowed in the command string.')
				);
	...
	onMount(async () => {
		...
		if (res) suggestionTags = res.map((tag: string) => ({ name: tag }));
	});
</script>
		...
		<div class="flex flex-col md:flex-row gap-4 flex-1 overflow-hidden pb-6">
			...
			<div class="hidden md:flex md:flex-col w-72 shrink-0 overflow-hidden">
				<div class="flex-1 overflow-y-auto">
					<!-- History Section (Svelte 3/4 compatible) -->
			...
			<div class="flex-1 flex flex-col min-h-0 overflow-hidden">
				<div class="flex items-center justify-between mb-1 shrink-0">
					<div class="flex items-center gap-2">
						...
						{#if selectedHistoryEntry}
							<span class="text-xs text-gray-500 font-mono bg-gray-100 dark:bg-gray-800 px-1.5 rounded">
				...
				<div class="relative flex-1 min-h-0">
					...
					>
						<pre class="text-xs whitespace-pre-wrap font-mono pr-8">
{selectedHistoryEntry?.snapshot?.content || content}
						</pre>
					</div>
				</div>
			</div>
