...
usage() {
    ...
    echo "  --olia[port=PORT]          Set the port for OLIA."
    ...
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --olia[port=3000]"
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --olia[port=3000] --data[folder=./ollama-data]"
    echo "  $0 --enable-gpu[count=1] --enable-api[port=12345] --olia[port=3000] --data[folder=./ollama-data] --build"
    echo ""
    echo "This script configures and runs a docker-compose setup with optional GPU support, API exposure, and OLIA configuration."
...
if [[ $kill_compose == true ]]; then
    ...
    if [[ -n $olia_port ]]; then
        export OLIA_PORT=$olia_port # Set OLIA_PORT environment variable
    fi
...
echo -e "   ${GREEN}${BOLD}OLIA Port:${NC} $olia_port"
