...
from olia.utils.payload import (
...
async def send_post_request(
    ...
    try:
        ...
        if r.ok is False:
            try:
                ...
                raise HTTPException(
                    ...
                    detail=f"OLIA: Server Connection Error",
                )
        ...
        raise HTTPException(
            ...
            detail=detail if e else "OLIA: Server Connection Error",
        )
    finally:
        if not streaming:
            await cleanup_response(r, session)
...
async def verify_connection(
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="OLIA: Server Connection Error"
            )
        except Exception as e:
...
async def get_openai_models(
    ...
    if url_idx is None:
        ...
        try:
            ...
            log.exception(e)
            error_detail = "OLIA: Server Connection Error"
            if r is not None:
                try:
                    res = r.json()
                    if "error" in res:
